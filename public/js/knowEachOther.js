$(function (){
	...
	jq('#img-show').banqh({
		...
		prev:"#prev1",//大图左箭头
		next:"#next1",//大图右箭头
		pop_div:"#popup-pic",//弹出框框架
		pop_pic:"#pop-pic",//弹出框图片框架
		pop_xx:".pop_up_close",//关闭弹出框按钮
		mhc:".mhc",//朦灰层
		...
		min_picnum: disp_num,//小图显示数量
		pop_up:false//大图是否有弹出框
	});

  jq(window).scroll(function (){

    var items = jq("#meet-content").find(".item");
    var menu = jq("#main-nav");
    ...
    if (top >= jq(document).height() - jq(window).height()) {
      jq('#topcontrol').addClass('backToTop');
    } else {
      jq('#topcontrol').removeClass('backToTop');
    }
  });
});
